    ...
    if(firebaseUser){
      ...
      btnSearch.addEventListener('click', e => {
        ...
        if(userInput.includes("@")){
        	buildingUI("email", userInput);
        }
        else if(userInput.includes(" ")){
        	var split_strings = userInput.split(" ");
        	var i;

        	if(buildingUI("last_name", toTitleCase(split_strings[split_strings.length - 1])) == "NO USER FOUND"){
        		alert("NO USER FOUND");
        	}
        	
        }
        else{
      //   	if(buildingUI("last_name", userInput) == "NO USER FOUND"){
    		// 	alert("NO USER FOUND");
    		// }

    		if(buildingUI("last_name", toTitleCase(userInput)) == false){
    			alert("NO USER FOUND");
    		}
    		else if(buildingUI("first_name", toTitleCase(userInput)) == false){
				alert("NO USER FOUND");
			}
...
function buildingUI(whatToSearch, userInput){

	// console.log(userInput.length);
	// console.log(userInput[userInput.length - 1]);
	firebase.database().ref('users').orderByChild(whatToSearch)
    .equalTo(userInput)
    .once('value', function(snap){
      ...
      if(foundUser != null){
        ...
        document.getElementById('info').appendChild(br1);
